...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (md5($_SESSION['taskId']) == $_POST['taskId']) {
    ...
    if ($error_message == '') {
      switch ($post_pattern) {
        case PostType::Edit:
          $result = mysql_query("SELECT password FROM molecule_posts WHERE topic_id = ".$topic_id." AND id = ".$post_id);
        ...
        case PostType::Reply:
          $result = mysql_query("SELECT max(id) max_id FROM molecule_posts WHERE topic_id = ".$topic_id);
          if (!$result) {
            echo mysql_error();
            echo "そのトピックは存在しません。";
            break;
          ...
          if ($error_message == '') {
            ...
            $sqlPost = "INSERT INTO molecule_posts(id, topic_id, writer, title, message, "
        ...
        case PostType::Topic:
          if (!mysql_query("INSERT INTO molecule_topics(updated) VALUES(now() + INTERVAL 14 HOUR)")) {
            //die(mysql_error());
            $error_message .= "データベースエラーが発生しました。";
          }
          if ($error_message == '') {
            $result = mysql_query("SELECT max(id) max_id FROM molecule_topics");
            $topic_id = 0;
            if (!$result) {
              //echo mysql_error();
              $error_message .= "データベースエラーが発生しました。";
...
switch ($input_type) {
  ...
  case InputType::Edit:
    ...
    $sqlForm = "SELECT writer, title, message, twitter_id, CASE mixi_id WHEN 0 THEN '' ELSE mixi_id END mixi_id, facebook_id, color FROM molecule_posts "
  ...
  case InputType::CommentReply:
    ...
    $sqlForm = "SELECT title, message FROM molecule_posts "
              ."WHERE topic_id = {$topic_id} AND id = {$post_id}";
    $rowsForm = mysql_query($sqlForm);
    if (!$rowsForm) {
      echo mysql_error();
